...
import { isTokenValid, login, register, update, destroy } from '../api/AuthEndpoints'
...
const AuthorizationContextProvider = props => {
  ...
  const signOutAndDestroy = async (onSuccess = null, onError = null) => {
    try {
      await destroy(loggedInUser)
      setLoggedInUser(null)
      if (Platform.OS === 'ios' || Platform.OS === 'android') {
        SecureStore.deleteItemAsync('user')
      } else {
  ...
  return (
    <AuthorizationContext.Provider value={{
      ...
      signOutAndDestroy: signOutAndDestroy
    }}
    >
      {props.children}
